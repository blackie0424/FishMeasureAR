...
```
拍照(點擊畫面設 A/B 兩點/單拍/連拍) → 確認測量線(靜態照片微調) → [比例尺換算(未量測照片)] → 比例尺編輯(參照物疊圖) → 表單(魚種*/漁法) → 統計(今日調查/CSV)
```

- **點哪量哪(構圖先行)**:先構好圖,直接點畫面上的吻端/尾叉設 A、B 點——手機全程不必移動,
  構圖不被量測打斷,追蹤飄移也降到最低。點位以世界座標錨定,數字氣泡貼線即時顯示。
- **快門後於凍結照片確認測量線**:AR 錨點在大幅移動時可能飄移(無 LiDAR 機型尤甚),
  確認頁上可拖曳端點對齊物體,長度等比修正;線+標籤以確認後端點於存檔時合成。
- **拍攝版面**:上下黑帶放全部控制項,中間 3:4 相機預覽只留數字氣泡;
  主按鈕=純快門(單拍需完成量測,連拍隨時可拍;Kit `CaptureControls`,有測試)。
